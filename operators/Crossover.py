...
class Crossover(ABC):
    ...
    def __init__(self, codage: Codage):
        self.codage = codage
...
class SimpleCrossover(Crossover):
    ...
    def __init__(self, codage):
        super().__init__(codage)

    def crossover(
        self, parent1: Individu, parent2: Individu
    ) -> Tuple[Individu, Individu]:
        """
        ...
        for num_coordonnee in range(nb_coordonnees):
            ...
            coord1.append(self.codage.decode(encoded_coord1))
            coord2.append(self.codage.decode(encoded_coord2))

        c1.coordonnees = np.array(coord1)
        c2.coordonnees = np.array(coord2)
...
class CrossoverMultipoint(Crossover):
    ...
    def crossover(
        self, parent1: Individu, parent2: Individu, nb_points: int
    ) -> Tuple[Individu, Individu]:
        """
        Tous les coordonnées codées sont de même longueur
        """
        ...
        taille_codage = len(encoded_parent1[0])
        assert (
            nb_points < taille_codage
        )  # Le nombre de points de crossover doit être inférieur à la taille du codage
        ...
        for num_coordonnee in range(nb_coordonnees):
            ...
            separateur = np.sort(
                np.random.choice(range(1, taille_codage), nb_points, replace=False)
                ...
                if i != 0 and i != len(separateur):
                    encoded_coord1 = np.concatenate(
                        (
                            encoded_coord1,
                            (
                                encoded_parent1[num_coordonnee][
                                    separateur[i - 1] : separateur[i]
                                ]
                                if i % 2 == 0
                                else encoded_parent2[num_coordonnee][
                                    separateur[i - 1] : separateur[i]
                                ]
                            ),
                        )
                    ...
                    encoded_coord2 = np.concatenate(
                        (
                            encoded_coord2,
                            (
                                encoded_parent2[num_coordonnee][
                                    separateur[i - 1] : separateur[i]
                                ]
                                if i % 2 == 0
                                else encoded_parent1[num_coordonnee][
                                    separateur[i - 1] : separateur[i]
                                ]
                            ),
                        )
                    ...
                    encoded_coord1 = np.concatenate(
                        (
                            encoded_coord1,
                            (
                                encoded_parent1[num_coordonnee][: separateur[0]]
                                if i == 0
                                else encoded_parent2[num_coordonnee][separateur[-1] :]
                            ),
                        )
                    )
                    encoded_coord2 = np.concatenate(
                        (
                            encoded_coord2,
                            (
                                encoded_parent2[num_coordonnee][: separateur[0]]
                                if i == 0
                                else encoded_parent1[num_coordonnee][separateur[-1] :]
                            ),
                        )
                    )
            c1.coordonnees_codees.append(encoded_coord1)
            ...
            c1.coordonnees = np.concatenate(c1.coordonnees, np.array(coord1))
            c2.coordonnees = np.concatenate(c2.coordonnees, np.array(coord2))
...
if __name__ == "__main__":
    ...
    simple = SimpleCrossover(codage)
    enfant1, enfant2 = simple.crossover(parent1, parent2)
    print(f"Enfant1 : {enfant1.coordonnees.coordonnees_codees}")
    print(f"Enfant2 : {enfant2.coordonnees.coordonnees_codees}")

    multiple = CrossoverMultipoint(codage)
    enfant3, enfant4 = multiple.crossover(parent1, parent2, 3)
